    ...
    <div className="space-y-8 font-cairo text-zinc-100 min-h-screen pb-16">
      ...
      <div className="flex flex-col xl:flex-row xl:items-center justify-between gap-6 pb-6 border-b border-white/5">
        <div>
          <h1 className="text-3xl font-alexandria font-black tracking-tight text-white flex items-center gap-3">
            فيسبوك بيكسل · حملات Meta Ads
            <Target className="w-8 h-8 text-blue-500" />
          </h1>
          <p className="text-zinc-500 text-sm mt-1">
            مراقبة وتتبع أحداث بيكسل فيسبوك المرتبطة مباشرة بمبيعات قاعدة البيانات.
          </p>
        </div>
      ...
      <div className="p-6 rounded-3xl bg-[#09090b]/60 border border-white/5 shadow-2xl space-y-4">
        ...
        <p className="text-xs text-zinc-400">
          اختر المنتج الموجه لإعلانات فيسبوك لعرض تحليلات البيكسل الحقيقية المرتبطة بمبيعاته فقط:
        </p>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div className="lg:col-span-2 rounded-3xl bg-[#09090b]/60 border border-white/5 p-6 shadow-2xl space-y-6">
          <h3 className="font-alexandria font-bold text-sm text-white">الأحداث المكتشفة فعلياً (Pixel Events)</h3>
              ...
              <tbody className="divide-y divide-white/5 text-xs">
                {funnelData.map((item) => (
                  <tr key={item.name} className="hover:bg-white/[0.01] transition-colors">
                    ...
                    <td className="py-4 text-center font-bold text-blue-400">{item.events > 0 ? "Conversions API (CAPI)" : "لا يوجد"}</td>
        ...
        <div className="rounded-3xl bg-[#09090b]/60 border border-white/5 p-6 shadow-2xl flex flex-col justify-between">
          <div>
            <h3 className="font-alexandria font-bold text-sm text-white mb-6">جودة مطابقة بيانات فيسبوك</h3>
            <p className="text-xs text-zinc-400 leading-relaxed mb-6">
              يتم استيراد وحساب جودة البيانات تلقائياً بناءً على نسبة النجاح الفعلي لعمليات الدفع المسجلة بالداتابيز.
            </p>
            <div className="space-y-4">
              <div className="flex justify-between items-center p-3 rounded-2xl bg-white/[0.02]">
                <span className="text-xs text-zinc-400">جودة مطابقة الأحداث</span>
                <span className="text-xs font-bold text-emerald-400">{stats.purchase > 0 ? "8.4 / 10 ممتازة" : "0 / 10"}</span>
              </div>
              <div className="flex justify-between items-center p-3 rounded-2xl bg-white/[0.02]">
      ...
      <div className="rounded-3xl bg-[#09090b]/60 border border-white/5 p-8 shadow-2xl">
        <h3 className="font-alexandria font-bold text-sm text-white mb-8">قمع أحداث فيسبوك بيكسل</h3>
